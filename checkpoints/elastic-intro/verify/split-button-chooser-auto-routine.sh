...
if jq -e '
	any(.tabs[]?.grid_layout.containers[]?;
		.type == "Split Button Chooser" and
		((.properties.topic // "") | endswith("AutoRoutine"))
	)
' "$LAYOUT" >/dev/null 2>&1; then
	echo "Split Button Chooser bound to AutoRoutine looks good."
	exit 0
fi

echo "Add a Split Button Chooser widget and bind it to AutoRoutine."
exit 1
